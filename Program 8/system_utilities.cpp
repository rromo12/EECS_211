...
int createfiles(char *toks[], int numberToks, char *ff){
	...
	int location, len, ret;
    location=findNode(toks[1]);
	len=string2int(toks[3]);
	if ( location == -1) return COMPUTER_NOT_FOUND;
	else 
		ret=networkArray[location]->createFile(toks[2],len, ff);
return ret;}
int findNode(char *n){
...
int ls(char *toks[], int numberToks){
	...
	location= findNode(toks[1]);
		if ( location == -1) return COMPUTER_NOT_FOUND;
		else networkArray[location]->printDirectory();
return 0; 	
}
int printfi(char *toks[], int numberToks){
	...
	location= findNode(toks[1]);
		if ( location == -1) return COMPUTER_NOT_FOUND;
		else networkArray[location]->printFiles(numberToks,toks);
		return 0;
}
int transfi(char *toks[], int numberToks){
int loc;
	loc=	findNode(toks[1]);
	if ( loc == -1) return COMPUTER_NOT_FOUND;
	networkArray[loc]->transfer_file(toks[3],toks[2]);
	return 0;
}
...
loc=	findNode(toks[1]);
	if ( loc == -1) return COMPUTER_NOT_FOUND;
	networkArray[loc]->deleteFiles(numberToks,toks);
	return 0;
}
